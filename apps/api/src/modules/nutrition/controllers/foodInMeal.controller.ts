...
export class FoodInMealController {
    ...
    create = async (req: Request, res: Response, next: NextFunction) => {
        const mealIdValidation = validateData(z.string().uuid(), req.params.mealId, 'Invalid meal ID')
        if (mealIdValidation.isFailure()) return next(mealIdValidation.error)

        const foodIdValidation = validateData(z.string().uuid(), req.params.foodId, 'Invalid food ID')
    ...
    findById = async (req: Request, res: Response, next: NextFunction) => {
        const idValidation = validateData(z.string().uuid(), req.params.id, 'Invalid item ID')
