...
import { createMealSchema, updateMealSchema } from "@ate-a-falha/shared"
...
export class MealController {
	...
	create = async (req: Request, res: Response, next: NextFunction) => {
		const dietIdValidation = validateData(z.string().uuid(), req.params.dietId, 'Invalid diet ID')
	...
	findById = async (req: Request, res: Response, next: NextFunction) => {
		const validation = validateData(z.string().uuid(), req.params.id, 'Invalid meal ID')
	...
	update = async (req: Request, res: Response, next: NextFunction) => {
		const idValidation = validateData(z.string().uuid(), req.params.id, 'Invalid meal ID in URL')
